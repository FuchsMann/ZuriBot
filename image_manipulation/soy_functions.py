...
class SoyFunctions:
    def soy(imageUrl: str) -> File:
        inImage = Image.open(BytesIO(requests.get(imageUrl).content))
        overImage = Image.open(Path("images", "soy.png"))
    ...
    def soyphone(imageUrl: str) -> File:
        ...
        if inImage.size[0] / inImage.size[1] >= 1:
            overImage = Image.open(Path("images", "soyphoneH.png"))
            ...
            overImage = Image.open(Path("images", "soyphoneV.png"))
            rootImage = Image.new("RGBA", overImage.size, (255, 255, 255))
